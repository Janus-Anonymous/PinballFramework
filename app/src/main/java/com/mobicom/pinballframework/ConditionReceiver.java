...
public class ConditionReceiver extends BroadcastReceiver {
     ...
     */
    HashSet<PinballRequest> DelayedRequest = new HashSet();
    /**
     * 被挂起的Request，这些是需要被捆绑处理的Request。
     */
    HashSet<PinballRequest> PendingRequest = new HashSet();
    /**
     * 唤醒机制，在指定时间唤醒设备
     */
     ...
     * @param eRequest
     */
    public void addRequest(PinballRequest eRequest) {
        /**
         * 处理Dozy类型的Request，只有在某些特定环境下才会延迟处理。
         */
        if (eRequest.getProperty() == PinballRequest.DOZY) {
         ...
         */
        else if (eRequest.getProperty() == PinballRequest.FROZEN) {
